...
void Timer5_Init(u16 arr,u16 psc)
{
	...
	TIM_ITConfig( TIM5, TIM_IT_Update|TIM_IT_Trigger, ENABLE );//TIM4 允许更新，触发中断
...
{
#if USMART_ENTIMX_SCAN==1
	Timer5_Init(1000,(u32)sysclk*100-1);//分频,时钟为10K ,100ms中断一次,注意,计数频率必须为10Khz,以和runtime单位(0.1ms)同步.
#endif
